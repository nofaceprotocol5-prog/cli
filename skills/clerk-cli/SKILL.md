...
If `clerk --version` reports a newer CLI than this skill (which is pinned to `{{CLI_VERSION}}`), run `clerk skill install` to refresh the bundled skill. The CLI binary is always the source of truth.
