...
- **`init` never selects or creates a real Clerk app for you in agent mode.** For keyless-capable frameworks, omit `--app` to use keyless. For frameworks that require API keys, pass `--app <id>` or link the project first.
